...
export const exportChart = {
  // Export chart as PNG
  async asPNG(element: HTMLElement, options: ExportOptions = { format: 'png' }): Promise<void> {
  ...
  async asSVG(element: HTMLElement, options: ExportOptions = { format: 'svg' }): Promise<void> {
  ...
  // Export chart as PDF
  async asPDF(element: HTMLElement, options: ExportOptions = { format: 'pdf' }): Promise<void> {
    const canvas = await html2canvas(element, {
      backgroundColor: options.backgroundColor || '#ffffff',
      scale: options.scale || 2,
    });
  ...
  async asExcel(data: any[], options: ExportOptions = { format: 'excel' }): Promise<void> {
  ...
  // Export data as CSV
  async asCSV(data: any[], options: ExportOptions = { format: 'csv' }): Promise<void> {
    const worksheet = XLSX.utils.json_to_sheet(data);
...
export const validateChart = {
  ...
  validateConfig: (config: Partial<ChartConfig>): { isValid: boolean; errors: string[] } => {
    ...
    if (
      config.type &&
      !['line', 'bar', 'pie', 'doughnut', 'radar', 'scatter', 'bubble'].includes(config.type)
    ) {
