  ...
  return (
    <div className="space-y-4">
      {events.map((event) => (
        <div
          key={event.id}
          className="bg-gray-800 flex items-start p-4 rounded shadow-md border-l-4 border-gray-600 transition-all hover:bg-gray-750"
        >
          <div
            className={`mt-1 h-3 w-3 rounded-full ${getSeverityColor(event.severity)} shadow-sm mr-4`}
          ></div>
            ...
            <div className="mt-2 flex gap-4 text-xs">
              {event.sourceIp && (
                <span className="text-gray-400">
                  IP: <span className="text-gray-200">{event.sourceIp}</span>
                </span>
              )}
              {event.userId && (
                <span className="text-gray-400">
                  User: <span className="text-gray-200">{event.userId}</span>
                </span>
              )}
            </div>
