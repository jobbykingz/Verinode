...
const ProofMetrics: React.FC<ProofMetricsProps> = ({
  ...
  const generateMockProofs = useMemo(() => {
    ...
    const count =
      timeRange === '24h' ? 100 : timeRange === '7d' ? 500 : timeRange === '30d' ? 2000 : 5000;
    ...
    for (let i = 0; i < count; i++) {
      ...
      mockProofs.push({
        ...
        verifier:
          status === 'verified' ? issuers[Math.floor(Math.random() * issuers.length)] : undefined,
      });
    }
    ...
    const loadProofs = async () => {
      ...
      if (selectedFilters.type && selectedFilters.type.length > 0) {
        filteredProofs = filteredProofs.filter((proof) =>
          selectedFilters.type.includes(proof.type),
        );
      }

      if (selectedFilters.status && selectedFilters.status.length > 0) {
        filteredProofs = filteredProofs.filter((proof) =>
          selectedFilters.status.includes(proof.status),
        );
      }

      if (selectedFilters.issuer && selectedFilters.issuer.length > 0) {
        filteredProofs = filteredProofs.filter((proof) =>
          selectedFilters.issuer.includes(proof.issuer),
        );
      }
    ...
    datasets: [
      {
        ...
        data: [...new Set(proofs.map((p) => p.type))].map(
          (type) => proofs.filter((p) => p.type === type).length,
        ),
        backgroundColor: [
          'rgba(59, 130, 246, 0.8)',
          'rgba(147, 51, 234, 0.8)',
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          ...
          <ChartLibrary type="pie" data={typeDistributionData} height={300} className="w-full" />
        </motion.div>
        ...
        >
          ...
          <ChartLibrary type="line" data={processingTimeData} height={300} className="w-full" />
        </motion.div>
        ...
        >
          ...
          <ChartLibrary type="bar" data={gasUsageData} height={300} className="w-full" />
        </motion.div>
      </div>
