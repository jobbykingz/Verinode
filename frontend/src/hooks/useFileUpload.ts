...
export const useFileUpload = (config: Partial<UploadConfig> = {}): UseFileUploadReturn => {
  ...
  useEffect(() => {
    const listeners = [
      ...
      uploadService.addEventListener('uploadStarted', (data) => {
        ...
        setFiles((prev) =>
          prev.map((f) =>
            f.id === fileId ? { ...f, status: { ...f.status, state: 'uploading' } } : f,
          ),
        );
        setIsUploading(true);
      }),
      ...
      uploadService.addEventListener('uploadProgress', (data) => {
        ...
        if (currentFiles.length > 0) {
          const totalProgress =
            currentFiles.reduce((sum, f) => sum + f.progress.percentage, 0) / currentFiles.length;
      ...
      uploadService.addEventListener('uploadCancelled', (data) => {
        ...
        setFiles((prev) =>
          prev.map((f) =>
            f.id === fileId ? { ...f, status: { ...f.status, state: 'cancelled' } } : f,
          ),
        );
  ...
  const upload = useCallback(
    async (options: UploadOptions): Promise<UploadFile[]> => {
      try {
        setError(null);
  ...
  const clear = useCallback(() => {
    ...
    setFiles((prev) =>
      prev.filter((f) => f.status.state !== 'completed' && f.status.state !== 'failed'),
    );
  }, [uploadService]);
  ...
  const getEstimatedTimeRemaining = useCallback(() => {
    ...
    const totalRemaining = uploadingFiles.reduce(
      (sum, f) => sum + (f.progress.totalBytes - f.progress.bytesUploaded),
      0,
    );

    const averageSpeed =
      uploadingFiles.reduce((sum, f) => sum + f.progress.speed, 0) / uploadingFiles.length;
  ...
  useEffect(() => {
    ...
    if (existingUploads.length > 0) {
      const totalProgress =
        existingUploads.reduce((sum, f) => sum + f.progress.percentage, 0) / existingUploads.length;
      setProgress(totalProgress);
    }
  }, [uploadService]);
