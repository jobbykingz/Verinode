...
const PerformanceMonitor: React.FC = () => {
  ...
  const vitals = [
    {
      name: 'FCP',
      val: `${currentVitals.fcp}s`,
      color: 'text-emerald-400',
      icon: <Zap className="w-4 h-4" />,
    },
    {
      name: 'LCP',
      val: `${currentVitals.lcp}s`,
      color: 'text-blue-400',
      icon: <Globe className="w-4 h-4" />,
    },
    {
      name: 'CLS',
      val: `${currentVitals.cls}`,
      color: 'text-amber-400',
      icon: <Gauge className="w-4 h-4" />,
    },
    {
      name: 'FID',
      val: `${currentVitals.fid}ms`,
    ...
    <div className="p-8 bg-slate-950 border border-slate-900 rounded-3xl shadow-2xl relative overflow-hidden group">
      ...
      <div className="flex justify-between items-start mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1 font-medium tracking-wide">
            Live bundle and rendering health analysis
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
        {vitals.map((v, i) => (
          <div
            key={i}
            className="p-5 bg-slate-900/40 border border-slate-800/50 rounded-2xl group-hover:border-blue-500/30 transition-all"
          >
      ...
      <div className="mt-8 flex items-center justify-between text-xs font-medium text-slate-500 bg-slate-900/40 p-4 rounded-xl">
        <div className="flex items-center">
          ...
          Optimized Bundle:{' '}
          <span className="text-blue-400 ml-1 font-bold">{currentVitals.bundleSize}</span>
        </div>
        <div className="flex items-center">
